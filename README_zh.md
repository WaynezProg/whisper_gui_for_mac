...
4. **翻譯結果**
   ...
   - 選擇目標語言
   - 需要設定 OpenAI API Key
   - ⚠️ **重要提醒**：AI 翻譯可能包含錯誤，請仔細檢查和驗證翻譯結果
...
1. **AI 準確性提醒**: 語音轉錄和 AI 翻譯均由 AI 模型驅動，可能包含錯誤。請在使用於重要用途前，仔細檢查和驗證所有結果。
2. **API Key 安全**: 請勿將 API Key 提交到公開倉庫，使用 `.env` 檔案管理
3. **模型檔案大小**: Whisper 模型檔案較大（數 GB），請確保有足夠空間
4. **編譯時間**: 編譯 whisper.cpp 可能需要 10-20 分鐘
5. **系統需求**: CoreML 模式需要 Apple Silicon（M1/M2/M3）或支援 CoreML 的 Mac
6. **外部依賴**: 應用程式需要用戶自行安裝和配置 `whisper.cpp` 和模型檔案，這些外部資源不會被打包到應用程式中
